...
def runEvaluation(pNum, maxNumFiles, dNum=1, scanOnly=False, scan=True, no_overwrite=True):
    ...
    if os.path.isfile(scorefile) and no_overwrite:          # if file already exists, load table
        ...
        if "Unnamed: 0.1" in table.columns.values:
            table.drop("Unnamed: 0.1", inplace=True, axis=1)
        if "Unnamed: 0" in table.columns.values:
    ...
    for targetNum in targetList:                            # test model on every target in the list
        ...
        for col in line.keys():
            table.at[0, col] = line[col]
            #table.at[table['Data Set']==tName, col] = line[col]
        #table.loc[table['Data Set']==tName,[table.columns.values]] = line.values
        print(table[table['Data Set']==tName])
